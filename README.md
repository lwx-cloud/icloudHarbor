...
安装器管理的 `docker-compose.yml`、`.env.example` 和镜像。安装目录即使已经包含其他文件也
不会触发归属识别或阻止安装；`.env` 不存在时直接生成带常用默认参数的新文件，其他已有文件
原样保留。新 `.env` 会固定使用安装器生成的 `docker-compose.yml`，避免目录内旧名称的 Compose
文件影响后续命令。若存在同名旧容器，安装器会优先从其挂载和受支持环境参数恢复原配置，包括
通知和自动清理设置，不限制旧容器使用的镜像标签，也不会复制系统环境。只有管理文件是目录或符号
链接、路径危险、同名容器缺少必要挂载等无法安全继续的情况才会停止。安装器不会递归修改已有
照片库的属主或群晖 ACL；若当前 UID/GID 无法写入，`status` 会给出具体检查结果。
