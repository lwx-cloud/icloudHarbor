...
重复运行安装器只更新自己管理的 Compose 文件和镜像，不覆盖现有配置、数据库、Session、
凭据、照片或目录内的其他文件。安装目录非空不会触发项目归属识别；缺少 `.env` 时直接生成
权限为 `0600`、包含常用默认参数的新文件，并固定使用安装器生成的 `docker-compose.yml`，
避免旧名称的 Compose 文件接管后续命令。若存在同名旧容器，安装器会优先从其挂载和受支持
环境参数恢复原配置；通知参数和 `IH_AUTO_DELETE` 等设置会一并保留，不限制旧容器的镜像标签，
也不会复制系统环境。只有管理文件类型冲突、危险路径或必要挂载无效等情况才会停止。下面是
