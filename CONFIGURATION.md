...
一键脚本把执行命令时的部署目录和 `config/` 设为 `0777`，把生成的 `.env` 与
`docker-compose.yaml` 设为 `0666`。即使用 root 执行，普通本机用户也能读写、重命名或删除。
共享主机若在 `.env` 填写通知密钥，可自行收紧权限；Apple 密码不会写入 `.env`。
...
增加 `IH_DOWNLOAD_TIMEOUT=30` 后重建容器；启动时会把这个环境变量同步写入 `config.yaml`。
...
不需要重建数据库。为了补回旧版本可能跳过的极少数同名 Asset，升级后建议临时删除
`IH_UNTIL_FOUND`，执行一次不限制范围的完整同步，再恢复该参数。0.6.0 的断点文件绑定远端
资源身份，因此升级时遗留的旧格式 `.part` 不会续传，以免把旧资源片段接到新文件。
