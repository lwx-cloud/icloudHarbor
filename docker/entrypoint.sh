...
mkdir -p /config/credentials /config/database /config/sessions /config/locks /config/tmp
...
/usr/sbin/gosu "$IH_RUNTIME_UID:$IH_RUNTIME_GID" \
  chmod 0700 /config/credentials /config/database /config/sessions /config/locks /config/tmp
